...
export default function AdminLayout({ title, breadcrumb }: { title: string; breadcrumb?: string[] }) {
  ...
  return (
    <div className="h-screen flex bg-gray-50">
      ...
      <div className="hidden md:block shrink-0">
        <Sidebar collapsed={collapsed} onToggleCollapse={() => setCollapsed((c) => !c)} variant="admin" />
      </div>
        ...
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="absolute left-0 top-0 h-full animate-fade-in">
            <div className="relative h-full">
              <Sidebar collapsed={false} onNavigate={() => setMobileOpen(false)} variant="admin" />
              <button
